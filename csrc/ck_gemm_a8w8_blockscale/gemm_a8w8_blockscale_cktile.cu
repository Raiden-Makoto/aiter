                                                      ...
                                                      torch::Tensor& Y)
{
    ...
    TORCH_CHECK(XQ.dim() == 2 && WQ.dim() == 2 && Y.dim() == 2,
                "mixed-scale CKTile GEMM expects 2D XQ, WQ, and Y");
    ...
    TORCH_CHECK(x_scale.dtype() == at::ScalarType::Float,
                "mixed-scale CKTile GEMM requires FP32 scales");
    TORCH_CHECK(Y.dtype() == at::ScalarType::BFloat16,
                "mixed-scale CKTile GEMM currently requires BF16 output");

    return gemm_a8w8_blockscale_cktile_impl<TILE_FP32,
                                            TILE_BF16,
                                            MixedGemmConfig,
                                            MixedAQuantGroupSize,
                                            MixedBQuantGroupSize,
                                            true>(
        XQ, WQ, x_scale, w_scale, Y, true, 1);
}
